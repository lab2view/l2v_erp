...
var BootstrapNotifyDemo = (function () {
  var demo = function () {
    $('#bootstrap-notify-gen-btn').click(function () {
      ...
      var notify_content1 =
        "&lt;script&gt;<br>   $.notify({<br>      title:'Email: Erica Fisher',<br>      message:'Investment, stakeholders micro-finance equity health Bloomberg; global citizens climate change. Solve positive social change sanitation, opportunity insurmountable challenges...'<br>   },<br>   {<br>      type:'" +
        $('#bootstrap-notify-state').val() +
        "',<br>      allow_dismiss:" +
        $('#bootstrap-notify-dismiss').prop('checked') +
        ',<br>      newest_on_top:' +
        $('#bootstrap-notify-top').prop('checked') +
        ' ,<br>      mouse_over:' +
        $('#bootstrap-notify-pause').prop('checked') +
        ',<br>      showProgressbar:' +
        $('#bootstrap-notify-progress').prop('checked') +
        ',<br>      spacing:' +
        $('#bootstrap-notify-spacing').val() +
        ',<br>      timer:' +
        $('#bootstrap-notify-timer').val() +
        ",<br>      placement:{<br>        from:'" +
        $('#bootstrap-notify-placement-from').val() +
        "',<br>        align:'" +
        $('#bootstrap-notify-placement-align').val() +
        "'<br>      },<br>      offset:{<br>        x:" +
        $('#bootstrap-notify-offset-x').val() +
        ',<br>        y:' +
        $('#bootstrap-notify-offset-y').val() +
        '<br>      },<br>      delay:' +
        $('#bootstrap-notify-delay').val() +
        ' ,<br>      z_index:' +
        $('#bootstrap-notify-z-index').val() +
        ",<br>      animate:{<br>        enter:'animated " +
        $('#bootstrap-notify-enter').val() +
        "',<br>        exit:'animated " +
        $('#bootstrap-notify-exit').val() +
        "'<br>    }<br>  });<br> &lt;/script&gt;";
