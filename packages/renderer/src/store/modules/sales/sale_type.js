...
const getters = {
  saleTypes: (state) =>
    state.saleTypes
      ? JSON.parse(state.saleTypes)
      : [],
  saleType: (state) =>
    state.saleType
      ? JSON.parse(state.saleType)
      : null,
};
...
const actions = {
  ...
  getSaleType({ commit, getters }, id) {
    const saleType = getters.saleTypes.find(
      (p) => p.id.toString() === id
    );
...
const mutations = {
  ...
  UPDATE_SALE_TYPE(state, saleType) {
    let saleTypes = JSON.parse(state.saleTypes);
    const index = saleTypes.findIndex(
      (p) => p.id === saleType.id
    );
    if (index !== -1) {
  ...
  DELETE_SALE_TYPE(state, saleTypeId) {
    state.saleTypes = JSON.stringify(
      JSON.parse(state.saleTypes).filter(
        (p) => p.id !== saleTypeId
      )
    );
  },
